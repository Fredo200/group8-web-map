...
var view = new ol.View({
    ...
    center: [34.604966898646, -0.004682184643391],
    zoom: 17,
...
var floors = new ol.layer.Image({
            title: 'floors',
            source: new ol.source.ImageWMS({
                url: 'http://localhost:8080/geoserver/wms',
                params: {
                    'LAYERS': 'maseno:floors'
                },
                ratio: 1,
                serverType: 'geoserver'
            })
...
// Add the rooms layer to the overlays group
overlays.getLayers().push(rooms);

var greenHouse = new ol.layer.Image({
    title: 'green house',
    source: new ol.source.ImageWMS({
        url: 'http://localhost:8080/geoserver/wms',
        params: {
            'LAYERS': 'maseno:green house'
        },
        ratio: 1,
        serverType: 'geoserver'
    })
...
overlays.getLayers().push(greenHouse);

var powerLines = new ol.layer.Image({
    title: 'maseno 11kv ex kisian',
    source: new ol.source.ImageWMS({
        url: 'http://localhost:8080/geoserver/wms',
        params: {
            'LAYERS': 'maseno:maseno 11kv ex kisian'
        },
        ratio: 1,
        serverType: 'geoserver'
    })
...
overlays.getLayers().push(powerLines);

var openspaces = new ol.layer.Image({
    title: 'openspaces',
    source: new ol.source.ImageWMS({
        url: 'http://localhost:8080/geoserver/wms',
        params: {
            'LAYERS': 'maseno:openspaces'
        },
        ratio: 1,
        serverType: 'geoserver'
    })
...
overlays.getLayers().push(openspaces);
...
overlays.getLayers().push(parkinglots);

var paths = new ol.layer.Image({
    title: 'paths',
    source: new ol.source.ImageWMS({
        url: 'http://localhost:8080/geoserver/wms',
        params: {
...
overlays.getLayers().push(paths);

var poles = new ol.layer.Image({
    title: 'poles',
    source: new ol.source.ImageWMS({
        url: 'http://localhost:8080/geoserver/wms',
        params: {
...
var waterbody = new ol.layer.Image({
    title: 'waterbody',
    source: new ol.source.ImageWMS({
        url: 'http://localhost:8080/geoserver/wms',
        params: {
            'LAYERS': 'maseno:waterbody'
        },
        ratio: 1,
        serverType: 'geoserver'
    })
...
overlays.getLayers().push(waterbody);
...
// Add the forest layer to the overlays group
overlays.getLayers().push(forest);

// Add error handling for WMS layer loading with retry mechanism
rooms.getSource().on('imageloaderror', function() {
    console.error('Error loading the "rooms" WMS layer. Retrying...');
    setTimeout(function() {
        rooms.getSource().refresh(); // Retry loading the layer
    }, 3000); // Retry after 3 seconds
});

// Add the floors layer to the overlays group
overlays.getLayers().push(floors);
// Add the buildings layer to the overlays group
overlays.getLayers().push(buildings);
...
var zoom_ex = new ol.control.ZoomToExtent({
    extent: [
        34.59963962848799, -0.001491894108874,
        34.61074601252776, -0.009145852944906
    ]
});
...
function clear_all() {
    ...
    map.getView().fit(overlays.getLayers().getArray().reduce((extent, layer) => {
        return ol.extent.extend(extent, layer.getSource().getExtent());
    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
            function (position) {
                ...
                console.log('User Coordinates:', userCoordinates); // Log the coordinates for debugging

                const transformedCoordinates = ol.proj.fromLonLat(userCoordinates, 'EPSG:4326');
                console.log('Transformed Coordinates:', transformedCoordinates); // Log the transformed coordinates
...
// Add a button to trigger the location function
...
locationButton.style.padding = '5px'; // Reduced padding
locationButton.style.fontSize = '12px'; // Reduced font size
